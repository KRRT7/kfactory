...
class RoutePort:
    ...
    def from_instance_port(cls, inst: Instance, base: BasePort) -> RoutePort:
        ...
        materialized_base = base.transformed(inst.trans)
        return cls(
    ...
    @property
    def materialized_base(self) -> BasePort | None:
        return self._materialized_base
    ...
    def to_port(self, *, name: str | None = None, copy_info: bool = False) -> Port:
        if self._materialized_base is not None:
            base = self._materialized_base.__copy__()
            if name is not None:
                base.name = name
            return Port(base=base)
        base = self.base.__copy__()
        base.name = self.base.name if name is None else name
        base.trans = self.trans.dup()
        base.dcplx_trans = None
        if not copy_info:
            base.info = self.base.info
        return Port(base=base)
